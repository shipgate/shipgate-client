...
export default function ManageStaff() {
  ...
  const fetchStaff = async (page = 1) => {
    try {
      const data = await getUsers("staff", token!, page, pagination.limit)
      setStaff(data.users || [])
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Staff Members ({pagination.total || staff.length})</CardTitle>
      ...
      {pagination.pages > 1 ? (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card p-4">
          <p className="text-sm text-muted-foreground">
            Page {pagination.page} of {pagination.pages} • {pagination.total} total staff members
          </p>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => fetchStaff(Math.max(1, pagination.page - 1))} disabled={pagination.page <= 1 || loading}>
              Previous
            </Button>
            <Button variant="outline" size="sm" onClick={() => fetchStaff(Math.min(pagination.pages, pagination.page + 1))} disabled={pagination.page >= pagination.pages || loading}>
              Next
            </Button>
          </div>
        </div>
